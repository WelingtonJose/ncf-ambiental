...
export default function CoverageSearch() {
  ...
  async function searchCoverage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      let unit = units.find(
        (item) => normalize(item.city) === place && item.state === state.toUpperCase(),
      );
      ...
      if (!unit && (latitude === undefined || longitude === undefined)) {
        try {
          const response = await fetch(
            `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(`${locality}, ${state}, Brasil`)}&format=jsonv2&limit=1`,
          );
          ...
          /* O CEP continua válido mesmo sem coordenadas. */
        }
      }
      ...
      if (!unit && latitude !== undefined && longitude !== undefined) {
        ...
        const closest = candidates
          .map((item) => ({
            item,
            distance: distanceBetween(latitude!, longitude!, item.latitude, item.longitude),
          }))
  ...
  return (
    <div className="rounded-2xl border border-slate-200 border-t-4 border-t-wm-green bg-white p-7 shadow-sm">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="font-display text-2xl font-black text-wm-dark">Consulte sua região</h2>
          <p className="mt-1 text-xs font-bold text-slate-500">
            Confira a cobertura operacional da NCF
          </p>
        </div>
        <span className="rounded-lg bg-[#edf4eb] p-3 text-wm-green">
          <MapPin className="h-5 w-5" />
        </span>
      </div>
      <form onSubmit={searchCoverage} className="mt-6">
        <label className="text-xs font-black uppercase tracking-[.08em] text-wm-green">
        ...
        <button
          type="submit"
          disabled={loading}
          className="mt-4 w-full rounded-md bg-wm-gold px-5 py-3.5 text-sm font-extrabold text-wm-dark disabled:opacity-70"
        >

...
const companySectors = [
  ...
  },
  {
    ...
    label: "Obras e grandes geradores",
...
export default function Header({ onQuote }: { onQuote: () => void }) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 lg:px-10">
        ...
        >
          ...
          <div className="hidden text-left sm:block">
            ...
            <span className="mt-1 block text-[10px] font-semibold uppercase tracking-[.16em] text-slate-500">
        ...
        <nav className="hidden items-stretch gap-1 lg:flex" aria-label="Navegação principal">
          ...
          >
            ...
            {openMenu === "residencial" && (
              <div className="dropdown-wrap w-[390px]">
                <div className="dropdown-card">
                  {residentialItems.map((item) => (
                    <DropdownItem key={item.href} item={item} onClick={close} />
                  ))}
                </div>
              </div>
            )}
          ...
          >
            ...
            />
            {openMenu === "comercial" && (
              <div className="dropdown-wrap w-[700px] -translate-x-[48%]">
          ...
          <Link
            href="/sobre"
            className={`nav-link ${pathname === "/sobre" ? "nav-link-active" : ""}`}
          >
            Sobre a NCF
          </Link>
        </nav>
        ...
        <button
          onClick={onQuote}
          className="hidden rounded-md bg-wm-gold px-5 py-3 text-sm font-extrabold text-wm-dark transition hover:shadow-md lg:block"
        >
          Solicitar atendimento
        </button>
        <button
          onClick={() => setMobileOpen(!mobileOpen)}
          className="rounded-md border border-slate-200 p-2 text-wm-green lg:hidden"
      ...
      {mobileOpen && (
        <div className="max-h-[calc(100vh-76px)] overflow-auto border-t border-slate-200 bg-white px-5 py-5 lg:hidden">
          <Link href="/" onClick={close} className="mobile-link">
            Início
          </Link>
          <p className="mobile-heading">Residencial</p>
          {residentialItems.map((item) => (
            <Link key={item.href} href={item.href} onClick={close} className="mobile-sub-link">
              {item.label}
            </Link>
          ))}
          <p className="mobile-heading">Serviços comerciais</p>
          {corporateServices.map((item) => (
            <Link key={item.href} href={item.href} onClick={close} className="mobile-sub-link">
              {item.label}
            </Link>
          ))}
          <p className="mobile-heading">Comercial por setor</p>
          {companySectors.map((item) => (
            <Link key={item.href} href={item.href} onClick={close} className="mobile-sub-link">
              {item.label}
            </Link>
          ))}
          <Link href="/sobre" onClick={close} className="mobile-link">
            Sobre a NCF
          </Link>
          <button
            onClick={() => {
...
function MenuLabel({ label, active, open }: { label: string; active: boolean; open: boolean }) {
  return (
    <span className={`nav-link flex items-center gap-1 ${active ? "nav-link-active" : ""}`}>
      {label}
...
function DropdownItem({
  ...
  item: { label: string; description: string; icon: typeof Home; href: string };
  onClick: () => void;
}) {
  ...
  return (
    <Link
      href={item.href}
      onClick={onClick}
      className="flex w-full items-start gap-3 rounded-lg p-3 text-left transition hover:bg-[#f7f9f6]"
    >
